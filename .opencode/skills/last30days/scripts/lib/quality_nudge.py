...
def _is_instagram_silent_failure(config: dict, research_results: dict) -> bool:
    ...
    returned zero items. The canonical cause was SC's keyword reels endpoint
    (/v2/instagram/reels/search) 404'ing on every query in production as of
    2026-08; the skill now searches /v1/instagram/search/popular instead and
    treats its 404 "no popular page" as a no-results outcome. Pre-fix the user
    got no signal at all - no Instagram section in the brief, no error in the
    footer, just unexplained absence.
    """
...
def _build_nudge_text(
    ...
    if "instagram" in bonus_errored:
        free_suggestions.append(
            "Instagram returned 0 reels despite SC being configured. The "
            "skill searches SC's /v1/instagram/search/popular endpoint; if "
            "zero items still appear, the topic has no curated Popular page "
            "on Instagram. Try a single-word topic like the most distinctive "
            "noun in your query."
        )
