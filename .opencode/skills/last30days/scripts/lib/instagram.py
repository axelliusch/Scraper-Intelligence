...
def _parse_date(item: Dict[str, Any]) -> Optional[str]:
    ...
    # Try ISO string first (ScrapeCreators Instagram search returns this)
...
def _parse_items(raw_items: List[Dict[str, Any]], core_topic: str) -> List[Dict[str, Any]]:
    ...
    for raw in raw_items:
        ...
        # Author info -- 'owner' in search/popular, 'user' in user/reels
...
def search_instagram(
    ...
    Uses /v1/instagram/search/popular, which returns the topic's curated
    Popular-page posts (reels included). SC's keyword reels endpoint
    (/v2/instagram/reels/search) is documented but 404s on every query in
    production (as of 2026-08), so keyword search routes through Popular.
    ...
    try:
        data = http.get(
            f"{SCRAPECREATORS_BASE}/v1/instagram/search/popular",
        ...
        # SC's keyword reels endpoint (/v2/instagram/reels/search) is documented
        # but 404s on every query in production (as of 2026-08). /search/popular
        # is the working topic search; it returns HTTP 404 "not_found" when the
        # topic has no Popular page - a legitimate no-results outcome, not a
        # hard failure. Any other 404 (path-level) stays a real error.
        if getattr(e, "status_code", None) == 404:
            body = (e.body or "").lower()
            if any(marker in body for marker in ("not_found", "no posts", "no popular page", "no scrapeable")):
                _log(f"No Instagram popular page for '{core_topic}'")
    ...
    # Posts live in the 'posts' array (ScrapeCreators /search/popular response)
    raw_items = data.get("posts") or data.get("reels") or data.get("items") or data.get("data") or []

    # Follow the cursor to honor the depth budget; each page returns ~12 posts.
    wanted = config["results_per_page"]
    cursor = data.get("cursor")
    has_more = data.get("has_more")
    while len(raw_items) < wanted and has_more and cursor:
        try:
            more = http.get(
                f"{SCRAPECREATORS_BASE}/v1/instagram/search/popular",
                params={"query": core_topic, "cursor": cursor},
                headers=http.scrapecreators_headers(token),
                timeout=30,
                retries=2,
            )
        except Exception as page_e:
            _log(f"IG popular pagination failed: {page_e}")
            break
        next_items = more.get("posts") or more.get("reels") or more.get("items") or []
        raw_items.extend(next_items)
        cursor = more.get("cursor")
        has_more = more.get("has_more")

    # Limit to configured count
    raw_items = raw_items[:wanted]
